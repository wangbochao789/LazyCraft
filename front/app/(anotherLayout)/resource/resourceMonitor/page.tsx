    ...
    if (userCredentials) {
      const handleLogin = async () => {
        try {
          ...
          // 部署的时候需要将admin换成user
          setRedirectUrl(encodeURIComponent('/bcc/#/reasoning/online/user?noHeader=1&noLeftMenu=1&noTopbar=1'))
        }
        catch (error) {
            ...
            <div style={{ width: '100%', height: '100%' }}>
              {
                token && refreshToken && (
                  <iframe
                    src={`http://scaihpc.cadi.net:30001/#/?userToken=${token}&refreshToken=${refreshToken}&redirectUrl=${redirectUrl}&noLeftMenu=1`}
                    width="100%"
                    height="100%"
                  ></iframe>
                )
